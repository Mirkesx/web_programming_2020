const commands = {
    ...
    open: {
        com: open,
        help: "open <path> <br>Open the file passed as parameter."
    }
}
...
function ls(node, param = {}) {
    ...
    let result = "";
    for (child of node.children) {
        result += (["dir","upload"].includes(child.type)  ? "@" : "#") + child.name + (param === '-a' || param === '--all' ? ", id: " + child.id + ", type: " + child.type + "<br>" : " ");
    }
    return { node, result };
}
...
function cd(node, param) {
    ...
    if (dir.type === "dir" || dir.type === "upload") {
        console.log(dir);
        return { node: dir };
    }
...
function rm(node, param) {
    ...
    if (element && !["root", "etc", "usr", "home", "username"].includes(element.name)) {
        if (element.name !== "upload" && (element.parent && element.parent.id !== file_manager.upload.id)) {
            ...
            //console.log(element.name+'.'+element.ext);
            let path = element.name + '.' + element.ext;
            $.ajax({
                type: 'GET',
...
function open(node, param) {
    if (param === "") {
        return { node: file_manager.username };
    }
    let element = getLastNode(node, param);
    if (element) {
        switch (element.type) {
            case "file":
                createNano(element);
                break;
            case "dir":
                createFileSystem(element);
                break;
            case "img":
                createDrawer(element);
                break;
            case "rem_file":
                createReader(element);
                break;
        }
        return {node};
    } else {
        return { node, result: "Path non valido!" };
    }
}